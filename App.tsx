...
// --- Visual Processing Component (Fluid Siri Style) ---
const VoiceWave = ({ isActive, isSpeaking }: { isActive: boolean; isSpeaking: boolean }) => {
  if (!isActive) return null;
  
  // Define color palettes based on state
  const thinkingGlow = "bg-indigo-500/20";
  const talkingGlow = "bg-emerald-500/20";
  
  const thinkingGradients = [
    "from-indigo-600 via-purple-600 to-blue-400",
    "from-purple-500 via-blue-600 to-indigo-400",
    "from-blue-400 via-white to-purple-400"
  ];
  
  const talkingGradients = [
    "from-rose-500 via-amber-400 to-emerald-400",
    "from-emerald-400 via-cyan-400 to-pink-500",
    "from-yellow-300 via-white to-orange-400"
  ];
  
  const palette = isSpeaking ? talkingGradients : thinkingGradients;
  const glow = isSpeaking ? talkingGlow : thinkingGlow;
  ...
  return (
    <div className="relative flex items-center justify-center w-16 h-16 overflow-visible animate-in fade-in zoom-in-95 duration-1000">
      ...
      {/* Morphing Layers with Color Blending */}
...
// --- Setup Animation (Cooking / Food Cart themed) ---
const SetupAnimation = () => (
  <div className="flex items-center justify-center gap-2">
    ...
    <span className="text-[7px] font-black uppercase tracking-widest animate-pulse">Igniting Hub...</span>
  </div>
);
...
export default function App() {
  ...
  const [logs, setLogs] = useState<AgentLog[]>([
    {
      id: 'initial-linguistic-insight',
      agent: 'Linguistic',
      message: 'Info Captured:\n\nமயிலாப்பூர்ல அந்த ஜன்னல் வழியா சுட சுட பஜ்ஜி வாங்கி அடிச்சா, வேற லெவல் பாஸ்! செம்ம காரமான சட்னி, எப்பவும் கிரௌடு தான்... நம்ம ஊரு கெத்து!\n\nGetting piping hot bajjis through that window in Mylapore is next level, boss! Super spicy chutney and always crowded... it\'s the pride of our city!',
      status: 'resolved'
    }
  ]);
  ...
  const [explorerTab, setExplorerTab] = useState<'logs' | 'discovery' | 'hubs'>('discovery');
  const [notifications, setNotifications] = useState<Notification[]>([]);
  
  // Voice states
  const [isSpeaking, setIsSpeaking] = useState(false);
  const [isVoiceActive, setIsVoiceActive] = useState(false); // True while thinking or speaking
  ...
  const [chatHistory, setChatHistory] = useState<ChatMessage[]>([
    { id: '1', role: 'model', text: 'Vanakkam! I am your GeoMind assistant. Ask me anything about landmarks or food!' }
  ]);
    ...
    setShops(prev => {
      ...
      const vendors = myProfiles.map(p => {
        ...
        return {
          ...
          menu: p.menu,
          hours: p.hours,
          address: isLive ? "Live Mobile Node" : "Standard Hub"
        };
      });
  ...
  const handleGenerateBioAI = async () => {
    if (!regForm.name || !regForm.cuisine) {
      addLog('Linguistic', 'Need Name and Cuisine to generate bio.', 'failed');
      return;
    }
    setIsGeneratingBio(true);
    addLog('Linguistic', 'Orchestrating AI for catchy vendor bio...', 'processing');
    try {
      const bio = await generateVendorBio(regForm.name, regForm.cuisine);
      setRegForm(prev => ({ ...prev, description: bio }));
      addLog('Linguistic', 'AI Bio successfully anchored.', 'resolved');
    } catch (err) {
      addLog('Linguistic', 'Bio generation failed.', 'failed');
    } finally {
  ...
  const startDiscovery = async () => {
    ...
    setShops(prev => [...prev.filter(s => !s.id.startsWith('sync')), ...result.shops]);
  ...
  const handleBroadcastLive = async () => {
    ...
    // Start processing visual for broadcast
    setIsVoiceActive(true);
    
    try {
      const alert = await spatialAlertAgent(profile.name, location);
      if (alert.audioData) {
        playVoice(alert.audioData);
      } else {
        setIsVoiceActive(false);
      }
      ...
      addLog('Spatial', 'Broadcast initiation failed.', 'failed');
      setIsVoiceActive(false);
    } finally {
  ...
  return (
    <div className="flex h-screen w-screen bg-[#020202] text-slate-300 font-mono overflow-hidden">
      {/* Global CSS for Liquid Siri Morphing */}
      <style>{`
        @keyframes siri-liquid {
          0% { border-radius: 60% 40% 30% 70% / 60% 30% 70% 40%; transform: rotate(0deg) scale(1); }
          33% { border-radius: 30% 70% 70% 30% / 50% 60% 30% 60%; transform: rotate(120deg) scale(1.1); }
        ...
        @keyframes siri-liquid-alt {
          0% { border-radius: 30% 70% 70% 30% / 50% 60% 30% 60%; transform: rotate(360deg) scale(1.1); }
          50% { border-radius: 60% 40% 30% 70% / 60% 30% 70% 40%; transform: rotate(180deg) scale(0.9); }
          100% { border-radius: 30% 70% 70% 30% / 50% 60% 30% 60%; transform: rotate(0deg) scale(1.1); }
        }
        @keyframes siri-liquid-fast {
          0% { border-radius: 50%; transform: scale(1) rotate(0deg); }
          50% { border-radius: 40% 60% 40% 60%; transform: scale(1.2) rotate(180deg); }
          100% { border-radius: 50%; transform: scale(1) rotate(360deg); }
        }
        .animate-siri-liquid {
          animation: siri-liquid 8s linear infinite;
        }
        .animate-siri-liquid-alt {
          animation: siri-liquid-alt 12s ease-in-out infinite;
        }
        .animate-siri-liquid-fast {
          animation: siri-liquid-fast 4s cubic-bezier(0.4, 0, 0.2, 1) infinite;
        }
      ...
      <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[3000] flex flex-col gap-3 pointer-events-none">
        {notifications.map(n => (
          <div key={n.id} className="relative w-80 bg-black/90 backdrop-blur-xl border border-emerald-500/50 rounded-2xl p-4 shadow-2xl pointer-events-auto cursor-pointer group/notif" onClick={() => handleNotifClick(n)}>
            <button 
              onClick={(e) => {
                e.stopPropagation();
                setNotifications(prev => prev.filter(notif => notif.id !== n.id));
              }}
              className="absolute top-3 right-3 w-6 h-6 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/10 text-white/40 hover:text-white transition-all text-[10px]"
            >
              ✕
            </button>
            <div className="flex gap-4 items-center">
              ...
              <div>
                ...
                <p className="text-[11px] font-bold text-white pr-4">{n.message}</p>
              </div>
            </div>
          </div>
        ))}
          ...
          {userMode === 'explorer' ? (
            <div className="space-y-4">
              ...
              <div className="flex gap-1 bg-white/5 p-1 rounded-lg">
                <button onClick={() => setExplorerTab('logs')} className={`flex-1 py-2 text-[8px] font-black uppercase rounded-md ${explorerTab === 'logs' ? 'bg-white/10 text-white' : 'text-white/20'}`}>Intel</button>
                <button onClick={() => setExplorerTab('discovery')} className={`flex-1 py-2 text-[8px] font-black uppercase rounded-md ${explorerTab === 'discovery' ? 'bg-white/10 text-white' : 'text-white/20'}`}>Discovery</button>
                <button onClick={() => setExplorerTab('hubs')} className={`flex-1 py-2 text-[8px] font-black uppercase rounded-md ${explorerTab === 'hubs' ? 'bg-white/10 text-white' : 'text-white/20'}`}>Live Hubs</button>
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              {activeProfileId ? (
                <div className="p-5 rounded-2xl bg-white/5 border border-white/10 space-y-4">
                  ...
                  <div className="flex flex-col gap-1 p-3 bg-black/20 rounded-lg border border-white/5">
                    <div className="flex justify-between items-center">
                      <span className="text-[7px] font-black text-white/30 uppercase tracking-widest">Spatial Coords</span>
                      <span className={`text-[7px] font-black uppercase ${isUpdatingLocation ? 'text-indigo-400 animate-pulse' : 'text-emerald-400'}`}>
                        {isUpdatingLocation ? 'Syncing...' : 'Stable'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center font-mono">
                      <span className="text-[9px] text-white/60">LAT: {location.lat.toFixed(6)}</span>
                      <span className="text-[9px] text-white/60">LNG: {location.lng.toFixed(6)}</span>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={handleBroadcastLive} disabled={isSettingUpLive} className={`py-4 text-white text-[9px] font-black rounded-xl shadow-lg transition-all ${isCurrentlyLive ? 'bg-indigo-600' : 'bg-emerald-600'} flex items-center justify-center min-h-[52px]`}>
                      {isSettingUpLive ? <SetupAnimation /> : (isCurrentlyLive ? 'Update Broadcast' : 'Go Live Now')}
                    </button>
             ...
             explorerTab === 'logs' ? (
                ...
                <div className="space-y-3">
                  {discoveredShops.length === 0 ? (
                    <p className="text-[9px] text-white/20 uppercase text-center py-20 italic">Run Food Discovery to map spatial food nodes</p>
                  ) : (
                ...
                <div className="space-y-3">
                  {liveVendorShops.length === 0 ? (
                     <p className="text-[9px] text-white/20 uppercase text-center py-20 italic">No registered vendor nodes currently broadcasting live</p>
                  ) : (
                    liveVendorShops.map(s => (
                      <div key={s.id} onClick={() => handleShopSelect(s)} className="w-full p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/40 text-left hover:bg-emerald-500/10 transition-all group cursor-pointer flex justify-between items-center">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      {activeProfile?.menu.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center p-3 bg-white/5 rounded-lg border border-white/5">
                          ...
                          <div className="flex items-center gap-3">
                            <span className="text-[10px] text-emerald-400">Rs. {item.price}</span>
                      ...
                      <div className="flex gap-2 p-2 bg-white/5 rounded-xl border border-white/10">
                        <input value={newItem.name} onChange={e => setNewItem({...newItem, name: e.target.value})} placeholder="Item Name" className="flex-1 bg-transparent text-[10px] text-white outline-none px-2" />
                        <input type="number" value={newItem.price} onChange={e => setNewItem({...newItem, price: e.target.value})} placeholder="Price" className="w-20 bg-transparent text-[10px] text-white outline-none border-l border-white/10 px-2" />
      ...
      <div className="flex-1 relative">
        ...
        {activeShop && (
          <div className="absolute bottom-10 left-10 right-10 z-[1000] animate-in slide-in-from-bottom-5">
            <div className="max-w-3xl mx-auto bg-black/95 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-white/10 shadow-2xl flex flex-col gap-8 relative">
              <button onClick={() => setActiveShop(null)} className="absolute top-6 right-6 w-10 h-10 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full flex items-center justify-center text-white/40 hover:text-white transition-all z-20">✕</button>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {activeShop.menu.map((item, i) => (
                          <div key={i} className="text-[9px] font-black text-emerald-400 bg-emerald-500/5 border border-emerald-500/20 px-4 py-2 rounded-full uppercase flex items-center gap-2">
                            <span>{item.name}</span>
                            <span className="text-white/40">• Rs. {item.price}</span>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        <div className={`fixed bottom-6 right-6 z-[2500] transition-all duration-500 ${isChatOpen ? 'w-[400px] h-[600px]' : 'w-16 h-16'}`}>
          {!isChatOpen ? (
            <button onClick={() => setIsChatOpen(true)} className="w-full h-full bg-indigo-600 rounded-full flex items-center justify-center text-white shadow-2xl hover:scale-110">
            ...
            <div className="w-full h-full bg-[#080808] border border-white/10 rounded-[2rem] flex flex-col overflow-hidden animate-in zoom-in-95 shadow-2xl backdrop-blur-3xl">
              <div className="p-6 bg-white/5 border-b border-white/5 flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <div className="bg-white/5 p-1 rounded-lg flex border border-white/10">
                    <button onClick={() => setChatLang('en-US')} className={`px-2 py-0.5 text-[7px] font-black rounded transition-all ${chatLang === 'en-US' ? 'bg-indigo-600 text-white' : 'text-white/20'}`}>EN</button>
                    <button onClick={() => setChatLang('ta-IN')} className={`px-2 py-0.5 text-[7px] font-black rounded transition-all ${chatLang === 'ta-IN' ? 'bg-indigo-600 text-white' : 'text-white/20'}`}>TA</button>
                  </div>
              ...
              <div className="h-4 flex items-center justify-center py-12">
                 <VoiceWave isActive={isVoiceActive} isSpeaking={isSpeaking} />
              </div>
              <form onSubmit={async (e) => {
                ...
                }} className={`p-3 rounded-xl transition-all ${isListening ? 'bg-red-500 text-white' : 'bg-white/5 text-white/40'}`}>🎤</button>
                <input value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Inquire..." className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[10px] text-white outline-none" />
        ...
        {isRegistering && (
          <div className="absolute inset-0 bg-black/80 backdrop-blur-xl z-[4000] flex items-center justify-center p-6 overflow-y-auto">
            <div className="w-full max-w-2xl bg-[#0c0c0c] border border-white/10 rounded-[2.5rem] p-10 space-y-8 animate-in zoom-in-95 shadow-2xl relative">
              <button onClick={() => setIsRegistering(false)} className="absolute top-6 right-6 w-10 h-10 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full flex items-center justify-center text-white/40 hover:text-white transition-all z-20">✕</button>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <input value={regForm.name} onChange={e => setRegForm({...regForm, name: e.target.value})} placeholder="Shop Name" className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-xs text-white outline-none" />
                  </div>
                  <div className="space-y-1">
                    <p className="text-[8px] text-white/20 uppercase font-black">Cuisine Class</p>
                    <input value={regForm.cuisine} onChange={e => setRegForm({...regForm, cuisine: e.target.value})} placeholder="e.g. South Indian" className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-xs text-white outline-none" />
                  </div>
                  <div className="space-y-1">
                    <div className="flex justify-between items-center mb-1">
                      <p className="text-[8px] text-white/20 uppercase font-black">Asset Bio</p>
                      <button onClick={handleGenerateBioAI} disabled={isGeneratingBio} className={`text-[7px] font-black uppercase px-2 py-1 rounded border transition-all ${isGeneratingBio ? 'animate-pulse text-indigo-400' : 'bg-indigo-600 text-white'}`}>AI Generate</button>
                    </div>
                    <textarea value={regForm.description} onChange={e => setRegForm({...regForm, description: e.target.value})} placeholder="Mission description..." className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-xs text-white h-24 resize-none outline-none" />
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <div className="space-y-2 max-h-44 overflow-y-auto custom-scrollbar bg-white/5 border border-white/10 rounded-xl p-3">
                      {regForm.menu.map((m, i) => (
                        <div key={i} className="flex justify-between items-center text-[10px] p-2 bg-white/5 rounded-lg"><span className="text-white uppercase">{m.name}</span><span className="text-emerald-400">Rs. {m.price}</span></div>
                      ))}
                      <div className="flex gap-2 pt-2 border-t border-white/10">
                        <input value={newItem.name} onChange={e => setNewItem({...newItem, name: e.target.value})} placeholder="Item" className="flex-1 bg-transparent text-[10px] text-white outline-none" />
                        <input type="number" value={newItem.price} onChange={e => setNewItem({...newItem, price: e.target.value})} placeholder="Rs." className="w-20 bg-transparent text-[10px] text-white outline-none border-l border-white/10 px-2" />
              ...
              <div className="flex gap-4 pt-4 border-t border-white/10">
                <button onClick={() => setIsRegistering(false)} className="flex-1 py-4 text-[9px] font-black text-white/40 uppercase tracking-widest">Abort Link</button>
                <button onClick={handleRegister} className="flex-[2] py-4 bg-indigo-600 text-white text-[11px] font-black rounded-2xl shadow-xl">Finalize Hub Integration</button>
              </div>
            </div>
          </div>
        )}
